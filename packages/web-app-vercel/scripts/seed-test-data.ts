...
async function withRetry<T>(operation: () => Promise<T>, maxRetries = 5, delayMs = 2000): Promise<T> {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            const result: any = await operation();
            if (result && result.error && (result.error.message?.includes("fetch failed") || result.error.message?.includes("ENOTFOUND"))) {
            ...
            console.warn(`[RETRY] Attempt ${attempt} failed: ${error.message || JSON.stringify(error)}. Retrying in ${delayMs}ms...`);
