    ...
    try {
        ...
        if (shouldUseLocalSupabaseFallback()) {
            ...
            if (addToPlaylistIds.length > 0) {
                addedCount += await supabaseLocal.addPlaylistItemsByArticleId(userEmail, addToPlaylistIds, actualArticleId)
            }

            if (removeFromPlaylistIds.length > 0) {
                removedCount += await supabaseLocal.removePlaylistItemsByArticleId(userEmail, removeFromPlaylistIds, actualArticleId)
            }
